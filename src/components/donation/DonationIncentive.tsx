...
const DonationIncentive: React.FC<DonationIncentiveProps> = ({ amount }) => {
  ...
  const multiplier = Math.max(1, Math.floor(numericAmount));
  ...
  return (
    <div className="bg-gold-50 p-5 rounded-lg border border-gold-200 my-4 shadow-sm">
      ...
      <div className="flex items-center justify-between mt-3 px-2">
        ...
        <div className="flex flex-col items-center">
          <div className="text-sm font-medium text-gold-700 mb-1">${numericAmount > 0 ? numericAmount.toFixed(2) : '1.00'}</div>
          <div className="text-xs text-gold-600">{multiplier > 1 ? `${multiplier}× Chance!` : 'Basic Chance'}</div>
        </div>
      </div>
    </div>
  );
